...
def geom_interp():
    ...
    ax.text(21.185, 19.9, r'$S^{(j)}(X)$', fontsize=10, color='red', verticalalignment='center', horizontalalignment='right')
    ax.text(20.65, 19.4, r'$S^{(i)}(X)$', fontsize=10, color='red', verticalalignment='center', horizontalalignment='right')
    ax.arrow(max(x),min(y), 0, max(y)-min(y)-0.1, head_width=0.05, head_length=0.1, fc='red', ec='red')
    ax.arrow(min(x),min(y), max(x)-min(x)-0.1, 0, head_width=0.05, head_length=0.1, fc='red', ec='red')
    ax.set_xlabel(r'$X^i$')
    ax.set_ylabel(r'$X^j$')
    ax.grid(True)
